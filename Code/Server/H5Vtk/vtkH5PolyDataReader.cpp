...
vtkCxxRevisionMacro(vtkH5PolyDataReader, "$Revision: 1.5 $");
...
{
//  std::cout << "vtkH5PolyDataReader Constructor" << std::endl;
  this->FileName = NULL;
  this->HDFPath = NULL;
...
vtkH5PolyDataReader::~vtkH5PolyDataReader()
{
 ...
 // std::cout << "========> vtkH5PolyDataReader Destructor" << std::endl;
}

//----------------------------------------------------------------------------
int vtkH5PolyDataReader::FillOutputPortInformation(int port, vtkInformation* info)
{
...
#if 1
//----------------------------------------------------------------------------
vtkPolyData* vtkH5PolyDataReader::GetOutput()
{
  return this->GetOutput(0);
}

//----------------------------------------------------------------------------
vtkPolyData* vtkH5PolyDataReader::GetOutput(int idx)
{
 // std::cout << "++{vtkH5PolyDataReader::GetOutput} Ref Count: " << this->GetOutputDataObject(idx)->GetReferenceCount() << std::endl;
  return vtkPolyData::SafeDownCast(this->GetOutputDataObject(idx));
}

//----------------------------------------------------------------------------
void vtkH5PolyDataReader::SetOutput(vtkPolyData *output)
{
...
//----------------------------------------------------------------------------
int vtkH5PolyDataReader::RequestData( vtkInformation *vtkNotUsed(request),
                                      ...
                                      vtkInformationVector *outputVector)
{
  ...
  vtkPolyData* p = loadPolyData(fileId, this->HDFPath);
  if (NULL != p)
  {
...
//
// -----------------------------------------------------------------------------
vtkPolyData* vtkH5PolyDataReader::loadPolyData(hid_t fileId, const std::string &hdfpath)
{
  ...
  if (rootId > 0)
  {
    ...
    {
      if (dataObjectType.compare(H5_VTK_POLYDATA) != 0)
      {
        std::cout << "HDF Group " << hdfpath << " is NOT type vtkPolyData. It is " << dataObjectType << std::endl;
        err = H5Gclose(rootId);
        err = H5Vtk::H5Utilities::closeFile(fileId);
        this->HDFError = -1;
        return output;
    ...
    err = readCells(output, rootId, verts, H5_VERTICES);
    if (err == 1)
    {
    ...
    err = readCells(output, rootId, lines, H5_LINES);
    if (err == 1)
    {
    ...
    err = readCells(output, rootId, polys, H5_POLYGONS);
    if (err == 1)
    {
      output->SetPolys(polys);
      //      int ncells=polys->GetNumberOfCells();
      //      int size=polys->GetNumberOfConnectivityEntries();
      //      std::cout << "ncells: " << ncells << std::endl;
      //      std::cout << "size: " << size << std::endl;
      //      std::cout << "max id: " << polys->GetData()->GetMaxId() << std::endl;
    }
    polys->Delete();

    //Read the TRIANGLE_STRIPS from the file
    vtkCellArray *tris = vtkCellArray::New();
    err = readCells(output, rootId, tris, H5_TRIANGLE_STRIPS);
    if (err == 1)
    {
    ...
    // Read any FIELD_DATA
    hid_t gid = H5Gopen(rootId, H5_FIELD_DATA_GROUP_NAME, H5P_DEFAULT);
    if (gid > 0)
    {
    ...
    // Read CELL_DATA
    gid = H5Gopen(rootId, H5_CELL_DATA_GROUP_NAME, H5P_DEFAULT);
    if (gid > 0)
    {
    ...
    // Read any POINT_DATA
    gid = H5Gopen(rootId, H5_POINT_DATA_GROUP_NAME, H5P_DEFAULT);
    if (gid > 0)
    {
...
//
// -----------------------------------------------------------------------------
int vtkH5PolyDataReader::readCells(vtkPolyData* output, hid_t rootId, vtkCellArray* verts, const std::string &dsetname)
{
  int  ncells, i;
  herr_t err = -1;
...
//
// -----------------------------------------------------------------------------
void vtkH5PolyDataReader::PrintSelf(std::ostream& os, vtkIndent indent)
{
...
int vtkH5PolyDataReader::ReadPoints(hid_t parentId, const std::string &dsetName, vtkPointSet* ps)
{
...
//
// -----------------------------------------------------------------------------
vtkAbstractArray* vtkH5PolyDataReader::ReadArray(hid_t parentId, const std::string& dsetName)
{
  ...
  case H5T_INTEGER:
    if ( H5Tequal(typeId, H5T_STD_U8BE) || H5Tequal(typeId,H5T_STD_U8LE) ) {
       ...
       ALLOCATE_AND_READ_ARRAY(array, vtkInt, numComp, numElements, parentId, dsetName ,int32_t);
