...
herr_t H5Utilities::closeFile(hid_t &fileId)
{
  ...
  //  except files
  vtkTypeInt32 num_open = H5Fget_obj_count(fileId, H5F_OBJ_DATASET | H5F_OBJ_GROUP |
...
// -----------------------------------------------------------------------------
herr_t H5Utilities::getObjectType(hid_t objId, const std::string &objName, vtkTypeInt32 *objType)
{
  herr_t err=1;
  H5G_stat_t obj_info;
 ...
 */
herr_t H5Utilities::getGroupObjects(hid_t loc_id, vtkTypeInt32 typeFilter, std::list<std::string>& names)
{
  herr_t err=0;
  hsize_t numObjs = 0;
...
//
// -----------------------------------------------------------------------------
vtkTypeInt32 H5Utilities::createGroupsForDataset(const std::string &datasetPath, hid_t parent)
{
...
//
// -----------------------------------------------------------------------------
vtkTypeInt32 H5Utilities::createGroupsFromPath(const std::string &pathToCheck, hid_t parent)
{
    ...
    if (err < 0) {
      ...
      case H5T_INTEGER:
        ...
        if ( H5Tequal(typeId, H5T_STD_U8BE) || H5Tequal(typeId,H5T_STD_U8LE) ) {
          ...
          IMXAArray::Pointer ptr = H5Utilities::readH5Attribute<vtkTypeInt32>(fileId, const_cast<std::string&>(datasetPath), (*iter), dims);
          attributes[*iter] = ptr;
...
// -----------------------------------------------------------------------------
herr_t H5Utilities::objectNameAtIndex(hid_t fileId, vtkTypeInt32 idx, std::string &name)
{
  herr_t err = -1;
