...
// -----------------------------------------------------------------------------
herr_t H5Lite::openId( hid_t loc_id, const std::string& obj_name, vtkTypeInt32 obj_type)
{
...
//  Closes the given ID
// -----------------------------------------------------------------------------
herr_t H5Lite::closeId( hid_t obj_id, vtkTypeInt32 obj_type )
{
 switch ( obj_type )
 {
  case H5G_DATASET:
  ...
  if ( sid >= 0 )
  {
    ...
    if ( rank > 0)
    {
     // hsize_t _dims[rank]; // allocate space for the dimensions
      ...
      {
        dims[i] = static_cast<vtkTypeUInt64>(_dims[i]);
      }
                            ...
                            IMXAArray* array)
{
  ...
  vtkTypeInt32 rank = array->getNumberOfDimensions();
  std::vector<vtkTypeUInt64> dims(rank, 0);
 ...
 /* Open the dataset. */
  ...
  if ( did >= 0 ) {
    ...
    if ( spaceId > 0 ) {
        ...
        std::vector<vtkTypeUInt64> _dims(dims.size(), 0);
        for (unsigned int i = 0; i<dims.size(); ++i)
        {
          _dims[i] = static_cast<vtkTypeUInt64>(dims[i]);
        }
        switch(attr_type)
        {
        case H5T_STRING:
        ...
        case H5T_INTEGER:
          ...
          if ( H5Tequal(typeId, H5T_STD_U8BE) || H5Tequal(typeId,H5T_STD_U8LE) ) {
             data = MXAArrayTemplate<vtkTypeUInt8>::New(_dims.size(), &(_dims.front() ) );
           } else if ( H5Tequal(typeId, H5T_STD_U16BE) || H5Tequal(typeId,H5T_STD_U16LE) ) {
             data = MXAArrayTemplate<vtkTypeUInt16>::New(_dims.size(), &(_dims.front() ) );
           } else if ( H5Tequal(typeId, H5T_STD_U32BE) || H5Tequal(typeId,H5T_STD_U32LE) ) {
             data = MXAArrayTemplate<vtkTypeUInt32>::New(_dims.size(), &(_dims.front() ) );
           } else if ( H5Tequal(typeId, H5T_STD_U64BE) || H5Tequal(typeId,H5T_STD_U64LE) ) {
             data = MXAArrayTemplate<vtkTypeUInt64>::New(_dims.size(), &(_dims.front() ) );
           } else if ( H5Tequal(typeId, H5T_STD_I8BE) || H5Tequal(typeId,H5T_STD_I8LE) ) {
             data = MXAArrayTemplate<vtkTypeInt8>::New(_dims.size(), &(_dims.front() ) );
           } else if ( H5Tequal(typeId, H5T_STD_I16BE) || H5Tequal(typeId,H5T_STD_I16LE) ) {
             data = MXAArrayTemplate<vtkTypeInt16>::New(_dims.size(), &(_dims.front() ) );
           } else if ( H5Tequal(typeId, H5T_STD_I32BE) || H5Tequal(typeId,H5T_STD_I32LE) ) {
             data = MXAArrayTemplate<vtkTypeInt32>::New(_dims.size(), &(_dims.front() ) );
           } else if ( H5Tequal(typeId, H5T_STD_I64BE) || H5Tequal(typeId,H5T_STD_I64LE) ) {
             data = MXAArrayTemplate<vtkTypeInt64>::New(_dims.size(), &(_dims.front() ) );
          } else {
