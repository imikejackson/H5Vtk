 ...
 */
static MXA_EXPORT herr_t openId( hid_t loc_id, const std::string& objName, vtkTypeInt32 obj_type);
 ...
 * @return Standard HDF5 Error Conditions
 */
static MXA_EXPORT herr_t closeId( hid_t obj_id, vtkTypeInt32 obj_type );
...
{
  if (typeid(value) == typeid(vtkTypeInt8)) return "H5T_NATIVE_INT8";
  if (typeid(value) == typeid(vtkTypeUInt8)) return "H5T_NATIVE_UINT8";

  if (typeid(value) == typeid(vtkTypeInt16)) return "H5T_NATIVE_INT16";
  if (typeid(value) == typeid(vtkTypeUInt16)) return "H5T_NATIVE_UINT16";

  if (typeid(value) == typeid(vtkTypeInt32)) return "H5T_NATIVE_INT32";
  if (typeid(value) == typeid(vtkTypeUInt32)) return "H5T_NATIVE_UINT32";

  if (typeid(value) == typeid(vtkTypeInt64)) return "H5T_NATIVE_INT64";
  if (typeid(value) == typeid(vtkTypeUInt64)) return "H5T_NATIVE_UINT64";
...
static hid_t HDFTypeForPrimitive(T value)
{
  ...
  if (NULL != name && name[0] == 'l' ) {
    std::cout << "You are using 'long vtkTypeInt32' as a type which is not 32/64 bit safe. Suggest you use one of the MXATypes defined in <Common/MXATypes.h> such as vtkTypeInt32 or vtkTypeUInt32." << std::endl;
  }
  return -1;
}
                            ...
                            std::vector<T> &data)
{
  ...
  std::vector<vtkTypeUInt64>::size_type size = dims.size();
  //std::vector<hsize_t> _dims(size, 0);
  std::vector<hsize_t> _dims(size, 0);
  for (std::vector<vtkTypeUInt64>::size_type i = 0; i < size; ++i)
  {
    _dims[i] = static_cast<hsize_t>(dims[i]);
  }
  sid = H5Screate_simple( static_cast<vtkTypeInt32>(size), &(_dims.front()), NULL );
  if (sid < 0)
  {
    return sid;
  }
...
/**
 * @brief Writes the data of a povtkTypeInter to an HDF5 file
                            ...
                            T* data)
{
  ...
  //Create the DataSpace
  std::vector<vtkTypeUInt64>::size_type size = static_cast<std::vector<vtkTypeUInt64>::size_type>(rank);

  std::vector<hsize_t> _dims(size, 0);
  for (vtkTypeInt32 i = 0; i < rank; ++i)
  {
    _dims[i] = static_cast<hsize_t>(dims[i]);
  }
 ...
 * @param loc_id The Parent location to write the dataset
 * @param dsetName The Name to use for the dataset
 * @param data const char povtkTypeInter to write as a null terminated string
 ...
 * @param objName The Name of Object to write the attribute vtkTypeInto.
 * @param attrName The Name of the Attribute
 * @param rank The number of dimensions in the attribute data
 * @param dims The Dimensions of the attribute data
 * @param data The Attribute Data to write as a povtkTypeInter
                             ...
                             std::vector<T> &data )
{
  ...
  hsize_t* dimsPtr = 0x0;
  //size mismatch between hsize_t and size_t
  std::vector<vtkTypeUInt64>::size_type _size = dims.size();
  //hsize_t _dims[ _size ];
  std::vector<hsize_t> _dims(_size, 0);
  for (std::vector<vtkTypeUInt64>::size_type i = 0; i < _size; ++i)
  {
    _dims[i] = static_cast<hsize_t>(dims[i]);
  }
  dimsPtr = &(_dims.front() );

  sid = H5Screate_simple( static_cast<vtkTypeInt32>(dims.size()), dimsPtr, NULL );
  if ( sid >= 0 ) {
    /* Verify if the attribute already exists */
    has_attr = H5Lite::findAttribute( obj_id, attrName );
 ...
 * @param attrName The name of the Attribute
 * @param size The number of characters  in the string
 * @param data povtkTypeInter to a const char array
 * @return Standard HDF error conditions
 */
static MXA_EXPORT herr_t  writeStringAttribute(hid_t loc_id,
                              const std::string& objName,
...
/**
 * @brief Reads data from the HDF5 File vtkTypeInto a preallocated array.
...
/**
 * @brief Reads data from the HDF5 File vtkTypeInto an std::vector<T> object. If the dataset
...
// std::cout << "  Opening " << dsetName << " for data Retrieval.  " << std::endl;
  ...
  if ( did >= 0 ) {
    ...
    if ( spaceId > 0 ) {
      vtkTypeInt32 rank = H5Sget_simple_extent_ndims(spaceId);
      if (rank > 0) {
        std::vector<hsize_t> dims;
       ...
       // std::cout << "NumElements: " << numElements << std::endl;
        //Resize the vector
        data.resize( static_cast<vtkTypeInt32>(numElements) );
       // for (vtkTypeUInt32 i = 0; i<numElements; ++i) { data[i] = 55555555;  }
 ...
 * @param data The variable to store the data vtkTypeInto
...
/**
 * @brief Reads a string dataset vtkTypeInto the supplied string.
...
/**
 * @brief reads a nullterminated string dataset vtkTypeInto the supplied buffer. The buffer
 * should be already preallocated.
 * @param loc_id The parent group that holds the data object to read
 * @param dsetName The name of the dataset.
 * @param data povtkTypeInter to the buffer
 * @return Standard HDF error condition
 */
static MXA_EXPORT herr_t readStringDataset(hid_t loc_id,
                                           const std::string &dsetName,
                            ...
                            std::vector<T> &data)
{
  ...
  if ( obj_id >= 0)
  {
    ...
    if ( attr_id >= 0 )
    {
      ...
      hsize_t numElements = 1;
      for (std::vector<vtkTypeUInt64>::iterator iter = dims.begin(); iter < dims.end(); ++iter )
      {
        numElements *= *(iter);
      }
      //std::cout << "    Vector Attribute has " << numElements << " elements." << std::endl;
      data.resize( static_cast<signed vtkTypeInt32>(numElements) );
 ...
 * @param attrName The name of the Attribute
 * @param data The preallocated memory for the variable to be stored vtkTypeInto
 * @return Standard HDF5 error condition
 */
template <typename T>
...
/**
 * @brief Reads the Attribute vtkTypeInto a pre-allocated povtkTypeInter
 * @param loc_id
 * @param objName The name of the dataset
 * @param attrName The name of the Attribute
 * @param data The preallocated memory for the variable to be stored vtkTypeInto
...
/**
 * @brief Reads a string attribute from an HDF object vtkTypeInto a precallocated buffer
 * @param loc_id The Parent object that holds the object to which you want to read an attribute
 * @param objName The name of the object to which the attribute is to be read
 * @param attrName The name of the Attribute to read
 * @param data The memory to store the data vtkTypeInto
 * @return Standard HDF Error condition
 */
static MXA_EXPORT herr_t readStringAttribute(hid_t loc_id,
                                   const std::string& objName,
 ...
 * @param objName The name of the dataset
 * @param attrName The name of the attribute
 * @param rank (out) Number of dimensions is store vtkTypeInto this variable
 */
...
#ifdef H5LITE_USE_MXA_CONSTRUCTS
/**
 * @brief Writes the data contained within an IMXAArray object vtkTypeInto an hdf5 file
 ...
 * it represents a path YOU need to make sure all vtkTypeIntermediate groups are already
 * crreated.
...
/**
 * @brief Reads the data from an HDF5 file vtkTypeInto a newly allocated MXAArrayTemplate
 * object - PLEASE NOTE: You MUST call delete on the object or otherwise make
 * arrangements to clean up the povtkTypeInter otherwise memory leaks will occur.
 * @param loc_id The HDF5 file or group Id vtkTypeInto which to store the data
...
/**
 * @brief Reads the attribute data from an HDF5 file vtkTypeInto a newly allocated MXAArrayTemplate
 * object - PLEASE NOTE: You MUST call delete on the object or otherwise make
 * arrangements to clean up the povtkTypeInter otherwise memory leaks will occur.
 * @param loc_id The HDF5 file or group Id vtkTypeInto which to store the data
 * @param dsetName The name/path of the data set
